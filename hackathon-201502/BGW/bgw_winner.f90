...
complex(kind((1.0d0,1.0d0))) :: expected_ans_1, expected_ans_2
integer idx
...
! ngpown = ncouls / (simulates number of mpi tasks)
      ...
      ALLOCATE(matngmatmgpD(number_bands,ncouls*ngpown))
      ...
      sch2Di=0D0
      ALLOCATE(wxi(nfreqeval, number_bands))
      wxi=0D0
...
! JRD compute the static CH
     do n1=1,number_bands
        ...
        do my_igp = 1, ngpown
           ...
           do ig = 1, igmax
              idx=ig+igmax*(my_igp-1)
              matngmatmgpD(n1,idx) = rightvector(ig,n1) * mygpvar1
           enddo
        enddo
...
! JRD: Now do CH term
     ...
     ALLOCATE(schDt_array(number_bands,nFreq))
...
!$OMP PARALLEL DO PRIVATE(schDt,my_igp,ig)  SHARED(matngmatmgpD,I_epsR_array,I_epsA_array, schDt_array)


     do ifreq=1,nFreq
        do n1=1,number_bands
           schDt_array(n1,ifreq) = 0D0
        end do

        do idx = 1, ngpown*ncouls
           do n1=1,number_bands         
              schDt_array(n1,ifreq) = schDt_array(n1,ifreq)  + matngmatmgpD(n1,idx)*( I_epsR_array(idx,ifreq) -I_epsA_array(idx,ifreq) )
           end do
        end do
           !              enddo
     ...
     do n1=1,number_bands     
        ...
        do ifreq=1,nFreq
           ...
           if (ifreq .ne. 1) then 
              ...
              schDt_right = schDt
              schDt_left = schDt_array(n1,ifreq-1)
...
! The below two lines are for sigma1 and sigma3
            if (ifreq .ne. nFreq) then
              schDi(:) = schDi(:) - CMPLX(0.d0,pref(ifreq)) * schDt / ( wxi(:,n1)-cedifft_coh)
              schDi_corb(:) = schDi_corb(:) - CMPLX(0.d0,pref(ifreq)) * schDt / ( wxi(:,n1)-cedifft_cor)
            endif
            if (ifreq .ne. 1) then
              do iw = 1, nfreqeval
!These lines are for sigma2
                intfact=abs((wxi(iw,n1)-cedifft_zb_right)/(wxi(iw,n1)-cedifft_zb_left))
...
!These lines are for sigma4
                if (flag_occ) then
                  ...
                  if (intfact .gt. 1d4) intfact = 1d4
                  intfact = log(intfact)
                  schDt_lin3 = (schDt_left + schDt_lin2*(-wxi(iw,n1)-cedifft_zb_left))*intfact
                else 
                  schDt_lin3 = (schDt_left + schDt_lin2*(wxi(iw,n1)-cedifft_zb_left))*intfact
                endif
     ...
     expected_ans_1=(-16862103.1278241,0d0)
     expected_ans_2=(-36828835.9338165d0,-14463922.8990243d0)
     write(6,*) "Answer:", achDtemp_cor(1), achDtemp(1)
     write(6,*) "Expected:", expected_ans_1, expected_ans_2
     write(6,*) "Diff:",achDtemp_cor(1)-expected_ans_1, achDtemp(1)-expected_ans_2
     
     DEALLOCATE(achDtemp)
     DEALLOCATE(achDtemp_cor)
     
   end program hackakernel
